...
menu = {
    'btn_menu_analize' : ('Выбрать другой сервис', menu_analyze_proc), 
...
@bot.callback_query_handler(func=lambda call: type(call) == types.CallbackQuery and call.data in startup_menu.keys())
def menu_startup_keyboard_handler(call):
    print((lambda call: type(call) == types.CallbackQuery and call.data in startup_menu.keys())(call))
    btn = call.data
    print(btn)
...
@bot.callback_query_handler(func=lambda call: type(call) == types.CallbackQuery and call.data in change_service.keys())
